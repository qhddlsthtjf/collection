...
public class AddDemo {
	public static void main(String[] args) {
		...
		//avengers.add("홍길동");
		String[] heros = {"iron","thor","hulk","hawk"};//크기가 4인 스트링 배열의 선언 과 생성
		for (int i = 0; i < heros.length; i++) {
			avengers.add(heros[i]);
		}
		//자료구조는 저장과 검색이라는 기능을 위해 만들어진 클래스이다.
		//thor 가 avengers 회원인지 검색해보자.
		Scanner scanner = new Scanner(System.in);
		System.out.println("어벤저스 회원검색 이름입력");
		...
		//첫번째 요소 삭제
		avengers.remove(0); //인덱스 값을 파라미터로 전달하면 삭제 완료 
	}
}
